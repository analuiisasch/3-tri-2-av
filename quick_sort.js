...
function quickSort(arr, inicio, fim) {
    if (inicio < fim) {
        //Encontra o índice de participação
        let indiceParticipacao = participar(arr, inicio, fim);

        //Ordena os elementos antes e depois da participação
        quickSort(arr, inicio, indiceParticipacao -1); //Lado esquerdo
        quickSort(arr, indiceParticipacao + 1, fim) //Lado direito
    }
}
...
function particionar(array, inicio, fim) {
    ...
    return i + 1; //Retorna o índice pivô
}
